...
parser.add_argument(
    "--output_suffix", "-o", type=str, default="_segTRI_proba.nii.gz",
    help="result of the segmentation, default=_segTRI_proba.nii.gz")
parser.add_argument(
    "--model", "-m", type=str,
...
for image_path, subject in zip(test_df["image"], test_df["subject"]):
    ...
    nib.save(
        nib.Nifti1Image(output.transpose(1, 2, 3, 0).astype(np.float32), affine),
        os.path.join(
